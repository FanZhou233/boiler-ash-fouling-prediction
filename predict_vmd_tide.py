"""Normalized script generated from data_prediction_VMD+TiDE.py."""
...
for k, mode_k in enumerate(modes):
    ...
    # Use the TiDE model instead of TSMixer.
    model = TiDEModel(
    ...
    pred_k = predict_series(model).pd_series().values.flatten()
    mode_preds[k] = pred_k

# The following steps merge, evaluate, and visualize the predictions.
...
df_results.to_csv('VMD+TiDE_result.csv', index=False)
